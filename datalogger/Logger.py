...
class TLX00(object):
    ...
    def readConfigFile(self, filename):
            ...
            try:
                for sensor in self.config["sensors"]:
                    ...
                    if "name" in sensor:
                        name = sensor["name"]
                    logging.info(
                        "Adding Sensor from config file: %d %s %s"
                        % (sensorid, sensortype, name)
                    )
                    # Todo: Sensortype weg machen
                    if sensortype in ("TL-3TSN", "TSN-50E", "TSN-EXT44", "TSN-33MN"):
                        self.sensors[
                            sensorid
                        ] = datalogger.Sensor.ArexxTemperatureSensor(
                            sensorid, sensortype, name
                        )
                        ...
                        self.sensors[
                            sensorid + 1
                        ] = datalogger.Sensor.ArexxHumiditySensor(
                            sensorid + 1, sensortype, name
                        )
                    else:
            ...
            for c in self.config["calibration"]:
                try:
                    ...
                    if not sensorid in self.sensors:
                        logging.error(
                            "Calibration values found for sensor %i, but sensor not defined in config",
                            sensorid,
                        )
                        continue
                    for n, v in c["values"].items():
                        ...
                        logging.debug(
                            "Calibration value for sensor %d oder %d value %f"
                            % (sensorid, n, float(v))
                        )
                except Exception as e:
        ...
        if "config" in self.config:
            if "DetectUnknownSensors" in self.config["config"]:
                self.detectUnknownSensors = bool(
                    self.config["config"]["DetectUnknownSensors"]
                )
    ...
    def detectSensor(self, sensorid, name=None):
        ...
        if detected_sensor != False:
            logging.debug(
                "Detected Sensor: %s(%s)" % (detected_sensor.id, detected_sensor.type)
            )
            if name != None:
                ...
                for sensor in self.sensors.items():
                    if sensor[0] == detected_sensor.displayid:
                        ...
                        logging.info(
                            "Setting name of detected sensor to: %s"
    ...
    def validateSensorData(self, data, sensor):
        if (
            abs(time.time() - data["timestamp"]) > 4000
        ):  # On DST changes we can get 3600sec difference.
            logging.info(
                "validateSensorData: timestamp %s is stale. Is a buffering receiver used?",
        ...
        if cooked > sensor.valmax or cooked < sensor.valmin:
            logging.info(
                "validateSensorData: Datapoint %f outside range (%f/%f). Ignoring."
                % (cooked, sensor.valmin, sensor.valmax)
            )
            return False
        return True
    ...
    def findDevices(self):
        ...
        if self.devices is not None:
            ...
            for d in self.devices:
                ...
                d.lastTimeDelete = 0
                logging.info(
                    "Bus %d Address %d Port Number %d "
                    % (d.bus, d.address, d.port_number)
                )
            return True
    ...
    def initializeDevices(self):
        for d in self.devices:
            try:
                ...
                logging.info(
                    "Device on Bus %d Address %d Port Number %d uses Addresses %d/%d for in/out"
                    % (d.bus, d.address, d.port_number, d.inAddress, d.outAddress)
                )
                self.setTime(d)
            except Exception as e:
                logging.error(
                    "Error initializing device at Bus %d Address %d Port Number %d. Resetting and removing device"
                    % (d.bus, d.address, d.port_number)
                )
    ...
    def setTime(self, device):
        logging.debug(
            "Setting time for USB device at Bus %d Address %d Port Number %d"
            % (device.bus, device.address, device.port_number)
        )
        # set mode
        self.clearRequestBuffer()
        self.requestBuffer[
            0
        ] = 4  # Protocol.txt says with type 04 the time can be set on the device
        # put time in array
        ...
        try:
            device.write(
                device.outAddress, self.requestBuffer, 1000
            )  # write to device at defined addres, write the buffer with time data ,timeout is 1000s
            device.read(
                device.inAddress, 64, 1000
            )  # read at device address,get the 64 byte long message, timeout is 1000s
            device.lastTimeSync = int(
                time.time()
            )  # set the actuel time since when the last sync has been performed
        except Exception as e:
    ...
    def deleteDeviceData(self, device):
        logging.debug(
            "deleting internal Flash data of USB device at Bus %d Address %d Port Number %d"
            % (device.bus, device.address, device.port_number)
        )
        # set mode
        self.clearRequestBuffer()
        self.requestBuffer[
            0
        ] = 0x0D  # this mode will delete the flash memory of the device
        try:
    ...
    def parseData(self, data):
        ...
        while pos < 63:
            ...
            if (data[pos] == 9 or data[pos] == 10) and pos < 55:
                ...
                sensorid = int.from_bytes(
                    [data[pos + 1], data[pos + 2]], byteorder="little", signed=False
                )

                rawvalue = int.from_bytes(
                    [data[pos + 3], data[pos + 4]], byteorder="big", signed=False
                )
                ...
                signal = None
                if data[pos] == 10:
                    signal = int.from_bytes(
                        [data[pos + 9]], byteorder="little", signed=False
                    )
            ...
            if (data[pos] == 11 or data[pos] == 12) and pos < 53:
                ...
                rawvalue = int.from_bytes(
                    [data[pos + 5], data[pos + 6]], byteorder="big", signed=False
                )
                timestamp = int.from_bytes(
                ...
                if data[pos] == 12:
                    signal = int.from_bytes(
                        [data[pos + 11]], byteorder="little", signed=False
                    )
    ...
    def loop(self):
        ...
        while len(self.listeners) > 0:
            for dev in self.devices:
                logging.debug(
                    "Polling device at Bus %d Address %d Port Number %d"
                    % (dev.bus, dev.address, dev.port_number)
                )
                ...
                while True:
                    try:
                        ...
                        dev.write(
                            dev.outAddress, self.requestBuffer, 1000
                        )  # send request to read the sensors
                        time.sleep(0.01)
                        rawdata = dev.read(
                            dev.inAddress, 64, 1000
                        )  # request the result from logger
                        if rawdata[0] == 0 and rawdata[1] == 0:
                            # no new data
                            break
                        dev.lastTimeDataRead = int(
                            time.time()
                        )  # store new time of new retrieved data
                        datapoints = self.parseData(
                            rawdata
                        )  # method to get process buffer data into usable data
                        # notify listeners
                        for datapoint in datapoints:
                            ...
                            if sensorid not in self.sensors:
                                ...
                                if detected_sensor != False:
                                    if self.validateSensorData(
                                        datapoint, detected_sensor
                                    ):
                                        self.addSensor(detected_sensor)
                            if sensorid in self.sensors:
                                ...
                                if self.validateSensorData(datapoint, sensor):
                                    for l in self.listeners:
                                        l.onNewData(
                                            datapoint, sensor
                                        )  # invoke method to share new data to the listerners
                        ...
                        if dev.deviceErrors > 10:
                            logging.warn(
                                "Too many errors. Removing device on Bus %d Address %d Port Number %d"
                                % (dev.bus, dev.address, dev.port_number)
                            )
