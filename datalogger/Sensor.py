    ...
    def readDeviceXML(self):
        ...
        try:
            ...
            for dt in devxml.findall("devicetype"):
                ...
                ArexxSensorDetector.arexxDeviceInfo.append(
                    {
                        "type": dtype,
                        "unit": unit,
                        "m1": m1,
...
class ArexxTemperatureSensor(Sensor):
    def __init__(self, sensorid, manufacturerType, name):
        super().__init__(sensorid)
        self.setType("Temperature").setUnit("°C").setName(name).setManufacturerType(
            manufacturerType
        )
    ...
    def rawToCooked(self, raw):
        ...
        # fallback default
        logging.info(
            "Set Temperature Sensor Type in config for exact values: Sensor %s"
            % self.id
        )
...
class ArexxHumiditySensor(Sensor):
    def __init__(self, sensorid, manufacturerType, name):
        super().__init__(sensorid)
        self.setType("Humidity").setUnit("%RH").setName(name).setManufacturerType(
            manufacturerType
        )
